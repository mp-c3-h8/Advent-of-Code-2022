...
class Tetris:
    def __init__(self, blueprints: list[Piece], jet: str, w: int = 7, spawn_x: int = 2, spawn_y: int = 3) -> None:
        ...
        self.max_y: int = 0
        self.top_row: list[str] = ["."] * w

    def play(self, n: int) -> int:
        for _ in range(n):
            self.drop()
        return self.max_y
    ...
    def drop(self) -> None:
        ...
        new_max_y = pos[1] + piece.bound_y
        if new_max_y > self.max_y:
            self.max_y = new_max_y
            self.top_row = ["."] * self.w
            for p in piece.pixel:
                if p[1]+pos[1] == self.max_y:
                    self.top_row[p[0]+pos[0]-1] = "#"
        elif new_max_y == self.max_y:
            for p in piece.pixel:
                if p[1]+pos[1] == self.max_y:
                    self.top_row[p[0]+pos[0]-1] = "#"

    def find_period(self) -> None:
        base_period = len(self.blueprints) * len(self.jet)
        last = 0
        for i in range(1, base_period*5+2):
            self.drop()

            if "".join(self.top_row).count("#") == 7:
                print(i-last)
                last = i
...
# after examination of find_period() output:
# we get a periodically full top row (#######)
# period starts after 441 drops with a length of 1700
until_period = game.play(441)
period_height = game.play(1700) - until_period
periods_to_play = (1000000000000 - 441) // 1700
remaining = (1000000000000 - 441) % 1700
remaining_height = game.play(remaining)
ans = remaining_height + (periods_to_play-1)*period_height  # 1 period already played
print("Part 2:", ans)
